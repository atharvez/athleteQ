...
async def validate_qr(req: QRValidateRequest, user: dict = Depends(get_current_user)):
    ...
    if existing.data:
        ...
        raise HTTPException(status_code=400, detail="Athlete has already completed this test.")
    ...
    # Simulate a test result immediately since user doesn't want the queue
    test_val = round(random.uniform(4.5, 6.0), 3) if event["test_type"] == "20m_sprint" else round(random.uniform(10.0, 15.0), 3)
    speed = round((20 / test_val) * 3.6, 1) if event["test_type"] == "20m_sprint" else None
